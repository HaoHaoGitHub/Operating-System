...
int is_number(const char *str) {
    ...
    // the first char is allowed to be '-' when it is a negative number
    if (isdigit(c) == 0 && c != '-') {
        return 0;
    }
...
int *resolve(const char *exp, int *result) {
    ...
    for (int i = 0; i < operand_num; i++) {
        ...
        if (pid == -1) {
            ...
            int expResult[2];
            int bytes_read = (int) read(pipes[operand_index][0], buffer, MAX_LENGTH); /* Read inside of the buffer */
            buffer[bytes_read] = '\0';
         ...
         else { /* pid > 0 means in parent process */
    ...
    if (argc > 1) {
        ...
        if (fp == NULL) {
            ...
            /* The fgets() function reads at most one less than the number of 
               ...
               in the string str. Reading stops when a newline is found, at-end-of-file
               or error. */
            while (fgets(exp, MAX_LENGTH, fp) != NULL) {
                ...
                if (is_valid(exp)) {
                    resolve(exp, result); /* Main opeartion here */
                } else {
